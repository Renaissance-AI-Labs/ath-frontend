...
const languagePacks = {
  'zh-cn': {
    ...
    'crash.fairness.step1Desc': '前往“我的投注”或“全网投注”列表，找到您想要验证的那一局游戏，记录下“投注区块号”。',
    'crash.fairness.step2': '第二步：获取区块哈希',
    'crash.fairness.step2Desc': '点击投注列表中的区块号（或者前往区块浏览器 BscScan），搜索该区块号。在区块详情页底部找到“Hash”或“Block Hash”字段，点击复制完整的哈希值。',
    ...
    'crash.fairness.edgeLabel': '庄家优势 (House Edge %)',
    ...
    'crash.fairness.dataCompositionDesc': '用于验证的输入数据是由以下三个部分依次拼接而成的。注意：为了模拟智能合约中 <code>abi.encodePacked</code> 的行为，我们需要将数值类型（如区块号）转换为 16 进制并手动补齐至 32 字节（64 位字符），这确保了生成种子的一致性。',
  ...
  'zh-tw': {
    ...
    'crash.fairness.step1Desc': '前往“我的投注”或“全網投注”列表，找到您想要驗證的那一局遊戲，記錄下“投注區塊號”。',
    'crash.fairness.step2': '第二步：獲取區塊哈希',
    ...
    'crash.fairness.dataCompositionDesc': '用於驗證的輸入數據是由以下三個部分依次拼接而成的。注意：為了模擬智能合約中 <code>abi.encodePacked</code> 的行為，我們需要將數值類型（如區塊號）轉換為 16 進制並手動補齊至 32 字節（64 位字符），這確保了生成種子的一致性。',
    ...
    'crash.fairness.tp_step2Desc': '點擊下方按鈕打開 <b>第三方 Keccak256 計算器</b>。粘貼第一步複製的數據，<span class="text-highlight-gold">務必選擇 "Hex" 輸入類型</span>，計算並複製 <b>Output</b> 框中的哈希值 (Hash)。',
    'crash.fairness.toolLinkText': '在線 Keccak256 計算器',
    'crash.fairness.tp_step3Title': '第三步：驗證 Hex 轉 Decimal',
  ...
  'en': {
    ...
    'crash.fairness.step1Desc': 'Go to "My Bets" or "All Bets", find the round you want to verify, and note the "Bet Block Number".',
    'crash.fairness.step2': 'Step 2: Get Block Hash',
    'crash.fairness.step2Desc': 'Click the block number in the bet list (or go to BscScan) to view block details. Find the "Hash" or "Block Hash" field and copy the full hash.',
    ...
    'crash.fairness.edgeLabel': 'House Edge (%)',
    ...
    'crash.fairness.dataCompositionDesc': 'The input data is constructed by concatenating these three parts in order. Note: To simulate smart contract <code>abi.encodePacked</code> behavior, numeric types (like Block Number) must be converted to hex and padded to 32 bytes (64 chars) to ensure consistent seed generation.',
    ...
    'crash.fairness.tp_step2Desc': 'Click the button below to open the <b>third-party Keccak256 Calculator</b>. Paste the data from Step 1 into the Input box, <span class="text-highlight-gold">make sure to select "Hex" input type</span>, then copy the hash from the <b>Output</b> box.',
    'crash.fairness.toolLinkText': 'Online Keccak256 Calculator',
    'crash.fairness.tp_step3Title': 'Step 3: Verify Hex to Decimal',
  ...
  'ja': {
    ...
    'crash.fairness.step1Desc': '「自分のベット」または「全ベット」リストに移動し、検証したいゲームラウンドを見つけて、「ベットブロック番号」を記録します。',
    'crash.fairness.step2': 'ステップ2：ブロックハッシュの取得',
    'crash.fairness.step2Desc': 'ベットリストのブロック番号をクリック（またはブロックエクスプローラーBscScanに移動）し、そのブロック番号を検索します。ブロック詳細ページの下部にある「Hash」または「Block Hash」フィールドを見つけ、完全なハッシュ値をコピーします。',
    ...
    'crash.fairness.dataCompositionDesc': '検証用の入力データは、以下の3つの部分を順番に連結して構成されています。注意：スマートコントラクトの<code>abi.encodePacked</code>の動作をシミュレートするために、数値タイプ（ブロック番号など）を16進数に変換し、32バイト（64文字）に手動でパディングする必要があります。これにより、シード生成の一貫性が保証されます。',
    ...
    'crash.fairness.tp_step2Desc': '下のボタンをクリックして<b>サードパーティのKeccak256計算機</b>を開きます。ステップ1でコピーしたデータを貼り付け、<span class="text-highlight-gold">必ず入力タイプとして「Hex」を選択</span>し、<b>Output</b>ボックスのハッシュ値（Hash）を計算してコピーします。',
    'crash.fairness.toolLinkText': 'オンライン Keccak256 計算機',
    'crash.fairness.tp_step3Title': 'ステップ3：HexからDecimalへの変換を検証',
  ...
  'ko': {
    ...
    'crash.fairness.step1Desc': '"내 베팅" 또는 "모든 베팅" 목록으로 이동하여 검증하려는 게임 라운드를 찾아 "베팅 블록 번호"를 기록합니다.',
    'crash.fairness.step2': '2단계: 블록 해시 가져오기',
    'crash.fairness.step2Desc': '베팅 목록에서 블록 번호를 클릭(또는 블록 탐색기 BscScan으로 이동)하여 해당 블록 번호를 검색합니다. 블록 상세 페이지 하단에서 "Hash" 또는 "Block Hash" 필드를 찾아 전체 해시 값을 복사합니다.',
    ...
    'crash.fairness.dataCompositionDesc': '검증용 입력 데이터는 다음 세 부분을 순서대로 연결하여 구성됩니다. 주의: 스마트 계약의 <code>abi.encodePacked</code> 동작을 시뮬레이션하기 위해 숫자 유형(예: 블록 번호)을 16진수로 변환하고 32바이트(64자)로 수동으로 패딩해야 합니다. 이는 시드 생성의 일관성을 보장합니다.',
    ...
    'crash.fairness.tp_step2Desc': '아래 버튼을 클릭하여 <b>타사 Keccak256 계산기</b>를 엽니다. 1단계에서 복사한 데이터를 붙여넣고, <span class="text-highlight-gold">반드시 "Hex" 입력 유형을 선택</span>한 다음, <b>Output</b> 상자의 해시 값(Hash)을 계산하고 복사합니다.',
    'crash.fairness.toolLinkText': '온라인 Keccak256 계산기',
    'crash.fairness.tp_step3Title': '3단계: Hex에서 Decimal로 변환 검증',
